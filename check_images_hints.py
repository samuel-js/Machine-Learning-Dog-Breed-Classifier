...
#
#          Check images & report s: read them in, predict their
...
def get_input_args():
    ...
    parser.add_argument('--dogfile', type = str, default = 'dognames.txt', 
                        help = 'path to the file dognames')
    ...
    for idx in range(0, len(in_files), 1):
       ...
       # isn't an pet image file
        if in_files[idx][0] != ".":
           ...
           # Use split to extract the image name into image_name
           image_name = in_files[idx].split('_')
            
           # Creates temporary label variable to hold pet label_name extracted 
           pet_label = ""

           # Add words to pet_label if words are all letters and add space at the end 
...
def calculates_results_stats(results_dic):
    ...
    for key in results_dic:
        # Labels match exaclty
        if results_dic[key][2] == 1:
            results_stats['n_match'] += 1
        # Pet Image Label is a dodog and labels match counts correct breed   
        if sum(results_dic[key][2:]) == 3:
            results_stats['n_correct_breed'] += 1
         
        # Pet Image is a dog - counts number of dod images
        
        if results_dic[key][3] == 1:
            results_stats ['n_dogs_img'] += 1
               
            # Classifier classifies image as dog(pet image is a dog)  
            # counts number of correct classifications
           
            if results_dic[key][4] == 1:
                results_stats['n_correct_dogs'] += 1
             
            # Pet image label is no a dog
